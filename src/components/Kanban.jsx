...
export function Kanban() {
  ...
  const idContainer = useRef(3);
  const idCards = useRef(5);
    ...
    let handler = (e) => {
      ...
      containers.forEach((container1) => {
        ...
        if (formRefs.current[container1.id].contains(e.target)) {
          ...
          !containerRefs.current.contains(e.target) &&
          !formRefs.current[container1.id].contains(e.target)
        ) {
          setTrigger({ triggerForm: true, triggerBtn: true });

          if (
            cardTitleRefs.current[titleCard.carta] &&
            !cardTitleRefs.current[titleCard.carta].contains(e.target)
          ) {
                ...
                container.id === titleCard.conte
                  ? {
                      ...
                      cards: container.cards.map((card) =>
                        card.id ===
                          parseInt(
                            cardTitleRefs.current[titleCard.carta].id,
                            10
                          ) && card.editor === false
                          ? {
                              ...card,
          ...
          if (
            titleRefs.current[container1.id] &&
            !titleRefs.current[container1.id].contains(e.target)
          ) {
            setContainers((containers) =>
              containers.map((container) => {
                if (
                  container.id ===
                  parseInt(titleRefs.current[container1.id].id, 10)
                ) {
  ...
  const deleteContainer = (containerValue) => {
    const copyContainers = containers.filter(
      (container) => container.id !== containerValue
    );
  ...
  useEffect(() => {
    if (titleRefs.current[title.contenedor]) {
      ...
      titleRefs.current[title.contenedor].querySelector("input").value =
        title.title;
  ...
  useEffect(() => {
    ...
    if (cardTitleRefs.current[titleCard.carta]) {
      ...
      cardTitleRefs.current[titleCard.carta].querySelector("input").value =
        titleCard.title;
    }
  }, [titleCard]);
    ...
    setContainers((containers) =>
      containers.map((container) => {
        if (container.id === containerId) {
          return {
            ...container,
            isFormVisible: !container.isFormVisible,
            isBtnVisible: !container.isBtnVisible,
          };
        }
        return container;
      })
      ...
      prevContainers.map((container) => {
        if (container.id === containerId) {
          return {
            ...
            cards: container.cards.map((card) => {
              if (
                card.id === parseInt(cardTitleRefs.current[cardId].id, 10) &&
                card.editor === true
              ) {
                setTitleCard({
                  title: card.text,
      ...
      prevContainers.map((container) =>
        container.id === containerId
          ? {
              ...
              cards: container.cards.map((card) =>
                card.id === cardId && card.editor === false
                  ? { ...card, text: texts, modificar: false, editor: true }
                  : card
              ),
            }
          : container
  ...
  const handleDragEnd = (result) => {
    ...
    if (sourceDrop !== destinationDrop) {
      ...
      setContainers((prevContainers) => {
        ...
        if (!insert) {
          console.error(
            "Could not extract the card from the source container"
          );
