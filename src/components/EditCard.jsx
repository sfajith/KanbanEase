...
export function EditCard({container, index, deleteCard, card, editCard, editCardHanlder, handleRadioChange, editCardTitle, setCardTitle, cardFormHandler}) {
  return (
      ...
      <div className={`absolute flex flex-col items-start bg-gray-500 rounded-xl left-full ml-2 w-64 h-32 z-50 pt-3 pl-2  ${!card.editor ? '' : 'hidden'}`}>
        <button className="flex items-center text-gray-500 mb-1 p-1 rounded bg-gray-100 hover:bg-blue-400 hover:text-gray-50 font-semibold" onClick={() =>{setCardTitle(container.id, card.id)}} ><MdOutlineDoneOutline  className="mr-1" />Aplicar</button>

        <button className="flex items-center whitespace-nowrap bg-gray-100 p-1 rounded text-gray-500 hover:bg-blue-400 hover:text-gray-50 font-semibold" onClick={editCardHanlder}> <LuTags className="mr-1"/> Asignar Prioridad</button>
        ...
        <button className="flex items-center text-gray-500 mt-1 p-1 p- rounded bg-gray-100 hover:bg-red-600 hover:text-gray-50 font-semibold" onClick={() => {deleteCard(container.id, index);}}><MdOutlineDeleteForever className="mr-1" />Eliminar Tarjeta</button>
  </div>
